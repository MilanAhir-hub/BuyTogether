...
const MyProperties = () => {
    ...
    if (isError) return (
        <div className="bg-red-50 text-red-500 p-6 rounded-3xl border border-red-100 flex items-center gap-3">
            <AlertCircle size={20} />
        ...
        <div className="max-w-6xl mx-auto">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
                ...
                <Link to="/seller/add-property">
                    <Button className="rounded-full px-8 py-4 shadow-xl shadow-primary/20 flex items-center gap-2">
                        <Plus size={20} />
                        List New Property
                    </Button>
                </Link>
            ...
            {properties.length === 0 ? (
                <div className="text-center py-24 bg-white rounded-[40px] border border-dashed border-slate-300">
                    ...
                    <Link to="/seller/add-property">
                        <Button variant="outline" className="rounded-2xl px-10 py-4">Create First Listing</Button>
                    </Link>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {properties.map((property) => (
                        <div key={property.id} className="group bg-white rounded-[34px] border border-slate-100 overflow-hidden shadow-sm hover:shadow-2xl hover:shadow-secondary/10 transition-all">
